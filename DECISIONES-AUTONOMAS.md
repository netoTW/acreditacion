...
- **La tabla sigue llamándose `cargo` aunque ahora contenga roles** · cambian las filas, no el modelo: el grano `(rol, dimensión)` es el mismo de ADR-003 · renombrar es una migración de nombres, se hace cuando no cueste nada más.
- **El umbral reforzado y la criticidad viven en `bloque_ruta`, no en `evaluacion`** · el contenido se genera una vez por (dimensión, nivel) y es compartido: el mismo bloque es crítico para un rol y estándar para otro · si AIEP unificara la exigencia, se sube a `evaluacion`.
- **Cada bloque define DOS medallas (silver y gold) y el rango lo decide la ruta** · así el contenido no sabe nada de roles, que es lo que ADR-003 protege; la base verifica que la gold solo nazca en una dimensión crítica · —.
- **La matriz de un rol se inserta en UNA sentencia** · el candado que verifica que la distribución sume 1 es diferido y en autocommit eso significa "al terminar la sentencia": fila por fila fallaría con una suma incompleta · —.
- **`nivel_estandar_origen`: el módulo 1 declara el tramo 1 y el 2 el nivel del bloque** · con 2 módulos fijos no cabe un módulo por tramo; el anidamiento CNA sigue estando, expresado en el reparto de conceptos · —.
- **El desafío usa `origen_tipo='juego'`** · no porque sea un juego, sino porque ese valor ya tiene el CHECK que impide XP acreditable: la regla queda en la base y no en una convención · —.
- **El hueco del juego por dimensión se muestra en la pantalla del bloque** · la estructura que AIEP definió tiene cuatro piezas y hay que poder ver cuál falta; ocultarlo daría la impresión de que el bloque está completo · se reemplaza por el juego real en la fase 2.
